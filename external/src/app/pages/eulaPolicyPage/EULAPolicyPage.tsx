...
const url = `https://raw.githubusercontent.com/Asaayu/${repoName}/main/EULA.md`;

export default function EULAPolicyPage() {
    ...
    useEffect(() => {
        fetch(url)
            ...
            .catch((error) => {
                console.error(error);
                setPolicy(`An error occurred while loading the end-user license agreement, please go to ${url} to view it in full.`);
            });
    }, []);
    ...
    return (
        <Container maxWidth="lg" sx={{ display: 'flex', justifyContent: 'center', userSelect: 'none' }}>
            <Paper
                elevation={2}
                sx={{ width: 'max-content', maxWidth: '35rem', borderRadius: '0.75rem', overflow: 'hidden', px: 4, pt: 2, my: 5 }}
            >
                <TitleBar />
                ...
                <Box
                    ...
                    sx={{ my: 2 }}
                >
                    <Typography variant="h4">End-User License Agreement</Typography>
