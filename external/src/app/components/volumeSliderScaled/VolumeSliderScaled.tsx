...
export const VolumeSliderScaled = () => {
    ...
    const handleChange = async (_event: Event, newValue: number | number[]) => {
        ...
        player.player?.setVolume(scaledValue);
    };

    const vehicleRadio = useMemo(() => {
        return settings.vehicleRadio;
    }, [settings.vehicleRadio]);

    const playingCustomRadio = useMemo(() => {
        return player.playingCustomRadio;
    }, [player.playingCustomRadio]);

    const disabled = useMemo(() => {
        return vehicleRadio && !playingCustomRadio;
    }, [vehicleRadio, playingCustomRadio]);

    const disabledOverlaySaved = useMemo(() => {
        return (
            <Box
                position="absolute"
                display="flex"
            ...
            >
                <SettingsTooltip title="Disable the Custom Vehicle Radio option below to control playback volume." placement="top">
    ...
    const LowVolumeIconSaved = useMemo(() => {
        return <FaVolumeLow size={24} />;
    }, []);

    const HighVolumeIconSaved = useMemo(() => {
        return <FaVolumeHigh size={24} />;
    }, []);
    ...
    useEffect(() => {
        const updateVolume = async () => {
            ...
            const currentVolume = await player.player?.getVolume();
            if (currentVolume == null) return;
            const scaledVolume = unscale(currentVolume);
            if (scaledVolume === value) return;
